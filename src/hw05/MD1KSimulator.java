...
public class MD1KSimulator {
	...
	public MD1KSimulator(double l, double totaltime, double sr,double service_time) {
		runTime = totaltime;
		lambda = l;
		SR = sr;
		Ts = service_time;
...
//			System.out.println("counter: "+ counter);
		...
		public void action(int requests, Schedule schedule, Queue queue) {
			if (action==0 ){ 
				...
				if (requests == 1) {
					...
					schedule.insert(new Event(1, SR+ time, Ts, birthRate,GlobalQvalue));
				}
			...
			// if the event type is that of a death
				...
				if (requests >= 1) {
					...
					if (queue.head!=null) {
						...
						schedule.insert(new Event(1, start + SR, Ts, birthRate,GlobalQvalue));
					}
				}
			}
	...
	public Event makenewcurrentm(MD1KSimulator mm1f) {
	...
	public void controller(MD1KSimulator simulator) {
...
//		 int k_value, int l, double serivce_time, int simulation_time)		Scanner kb = new Scanner(System.in);
//		computeEvalue("(D)",  3, 50,0.030, 100);
		computeEvalue("(E)",3, 60, 0.015, 100);
...
//		double lambda, double totaltime, double sr,double service_time
		
		MD1KSimulator simulator = new MD1KSimulator(l, simulation_time, sr, serivce_time);
